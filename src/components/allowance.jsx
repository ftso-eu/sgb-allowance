        ...
        is721(contract, this.props.tx.allowanceUnEdited).then((result) => {
            if(result) {
                ...
                contract.methods.approve(0, this.props.tx.allowanceUnEdited).send({ from: this.props.account }).then((receipt) => {
                    ...
                    window.alert("Ooopsi! Something went wrong, press ok to reload the app. Please retry to revoke from Chrome and Metamask on your PC! Error details: " + JSON.stringify(err));
                    window.location.reload();
                });
            } else {
